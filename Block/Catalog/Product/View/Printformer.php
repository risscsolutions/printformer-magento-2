...
class Printformer extends AbstractView
{
    ...
    public function __construct(
        ...
        $this->draftMasterId
            = $this->_catalogSession->getData('printformer_masterid');
    }
    ...
    public function draftInUsage($printformerProduct)
    {
        ...
        if (!empty($draftId)) {
            switch ($this->getRequest()->getModuleName()) {
                case 'checkout':
                    $result
                        = $this->cartHelper->draftIsAlreadyUsedInCart($draftId);
                    break;
                case 'wishlist':
                    $result
                        = $this->cartHelper->draftIsAlreadyUsedInCurrentWishlist($draftId);
                    break;
            }
        }
    ...
    public function getJsonConfig()
    {
        ...
        $config = [
            ...
            'allowSkipConfig'                          => $this->configHelper->isAllowSkipConfig(),
            //@todo || $this->getDraftId(),
            'printformerProducts'                      => $this->getPrintformerProductsFrontendConfigurationSource(),
            'variationsConfig'                         => $this->getVariationsConfig(),
            'variations'                               => [],
            //@todo $this->getProductVariations($this->getDraftId()),
            'qty'                                      => $minSaleQty,
            ...
            'isAddToCartRedirect'                      => $this->configHelper->getConfigRedirect()
                != Redirect::CONFIG_REDIRECT_URL_PRODUCT,
        ];

        if ($this->getPersonalisations()) {
            $config[Save::PERSONALISATIONS_QUERY_PARAM]
                = $this->getPersonalisations();
        ...
        if (!isset($preselectData['product'])
            || $product->getId() != $preselectData['product']
        ) {
            $preselectData = [];
        }
     ...
     * @return array
     */
    public function getPrintformerProductsFrontendConfigurationSource(
        ...
        foreach ($pfProducts as $printformerProductKey => $printformerProduct) {
            $draftId
                = $this->cartHelper->searchAndLoadDraftId($printformerProduct);

            $printformerProducts[$printformerProductKey]
                = $printformerProduct->getData();
            $printformerProducts[$printformerProductKey]['url']
                = $this->getEditorUrl($printformerProduct, $product, $draftId);

            if (!empty($draftId)) {
                if ($this->canShowDeleteButton()) {
                    $printformerProducts[$printformerProductKey]['delete_url']
                        = $this->getDeleteUrl($printformerProduct,
                        $product->getId());
                }
                $printformerProducts[$printformerProductKey]['draft_id']
                    = $draftId;
                $personalisations
                    = $this->getPersonalisationCount($printformerProducts[$printformerProductKey]['draft_id']);
                if ($personalisations > 1) {
                    $printformerProducts[$printformerProductKey]['personalisations']
                        = $personalisations;
                }
            }
        }
    ...
    {
        return ($this->getRequest()->getModuleName() == 'checkout'
                ...
                switch ($this->getRequest()->getModuleName()) {
                    case 'checkout':
                        ...
                        if ($quoteItem
                            ...
                            $intent
                                        = $buyRequest->getData(InstallSchema::COLUMN_NAME_INTENT);
                        }
                        break;
                    case 'wishlist':
                        $wishlistItem
    ...
    public function getDraftMasterId()
    {
        if (!$this->draftMasterId
            ...
            if ($quoteItem && $productId == $quoteItem->getProduct()->getId()) {
                $this->draftMasterId = $quoteItem->getBuyRequest()
                    ->getData('printformer_masterid');
            }
        }
    ...
    public function getPersonalisations()
    {
        if ($this->getRequest()->getActionName() == 'configure'
            ...
            $catalogSession = $this->sessionHelper->getCatalogSession();
            $personalisations
                            = $catalogSession->getData(Save::PERSONALISATIONS_QUERY_PARAM);
            if (isset($personalisations[$this->getProduct()
                    ->getStoreId()][$this->getProduct()->getId()])
            ) {
                return $personalisations[$this->getProduct()
                    ->getStoreId()][$this->getProduct()->getId()];
            }
        }
    ...
    public function getConfigurePreselection()
    {
        ...
        if ($request->getParam('id')) {
            /** @var Item $quoteItem */
            $quoteItem = ObjectManager::getInstance()
                ->create('Magento\Quote\Model\Quote\Item');
            $quoteItem->getResource()->load($quoteItem,
            ...
            if ($quoteItem->getId()) {
                ...
                $productMeta
                    = $objm->get('Magento\Framework\App\ProductMetadataInterface');
                if (version_compare($productMeta->getVersion(), '2.2.1',
                    '>=')
                ) {
                    $buyRequest = new DataObject(json_decode($result['value'],
                        true));
                } else {
                ...
                $this->_eventManager->dispatch('printformer_get_preselection_after',
                    [
                        'connection'       => $connection,
                        'options'          => $options,
                        'json_data_object' => $jsonDataObject,
                    ]);
