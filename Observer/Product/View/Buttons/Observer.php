...
class Observer implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        $pfProduct
            = $this->getPfProductByMasterId($product->getPrintformerProduct(),
            $storeId);
        if ($pfProduct->getId()) {
            $attribute = $product->getResource()
                ->getAttribute('printformer_capabilities');

            if ($attribute->getId()) {
                $currentlySelectedValueFromMultiselect
                                    = $attribute->getFrontend()
                    ->getValue($product);
                $multiselectAsArray = explode(',',
                    $currentlySelectedValueFromMultiselect ?? '');
                $allOptionIds       = [];
                foreach ($multiselectAsArray as $singleMultiselectItem) {
                    $allOptionIds[] = $attribute->getSource()
                        ->getOptionId(ltrim($singleMultiselectItem));
                }

                $connection = $product->getResource()->getConnection();
                $table
                            = $connection->getTableName('catalog_product_entity_text');
