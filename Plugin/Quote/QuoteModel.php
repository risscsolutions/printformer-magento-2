...
class QuoteModel
{
    ...
    public function aroundAddProduct(
        ...
        if (!empty($draftIds)) {
            ...
            foreach ($draftHashArray as $draftId) {
                ...
                if ($isReordered) {
                    $oldDraftId = $draftId;
                    $customerId = $this->session->getCustomerSession()
                        ->getCustomerId();
                    $newDraftProcess
                                = $this->_apiHelper->generateNewReplicateDraft($oldDraftId,
                        $customerId);
                    if (!empty($newDraftProcess)) {
                        ...
                        if (!empty($newDraftId)) {
                            $draftId = $newDraftId;
                            $relations
                                     = $buyRequest->getData('draft_hash_relations');
                            if (!empty($relations[$newDraftProcess->getProductId()][$newDraftProcess->getPrintformerProductId()])) {
                                $relations[$newDraftProcess->getProductId()][$newDraftProcess->getPrintformerProductId()]
                                    = $newDraftId;
                                $buyRequest->setData('draft_hash_relations',
                                    $relations);
                            }
                        }
                    }
                ...
                if ($draftProcess->getId()) {
                    $draftHashRelations
                        = $this->cartHelper->updateDraftHashRelations(
                        $draftHashRelations,
            ...
            $buyRequest->setData(InstallSchema::COLUMN_NAME_DRAFTID,
                $newDraftHashArrayFormatted);
        ...
        return is_string($result) ? $result
            : $this->setPrintformerData($result);
    }
     ...
     * @return void
     */
    private function loadPrintformerDataByQuoteItem(
        SubjectQuote\Item\AbstractItem $quoteItem
    ) {
        try {
            ...
            if (!empty($buyRequest)) {
                if (isset($buyRequest[InstallSchema::COLUMN_NAME_DRAFTID])) {
                    ...
                    $draftHashRelations
                                = $buyRequest->getData('draft_hash_relations');
                    if (isset($draftHashRelations)
                        && is_array($draftHashRelations)
                    ) {
                        if (isset($draftHashRelations[$quoteItem->getProduct()
                                ->getId()])
                        ) {
                            $newDraftHashes
                                               = $draftHashRelations[$quoteItem->getProduct()
                                ->getId()];
                            $newDraftHashField = implode(',', $newDraftHashes);
                            $quoteItem->setData(InstallSchema::COLUMN_NAME_DRAFTID,
                                $newDraftHashField);
                            $quoteItem->setData(InstallSchema::COLUMN_NAME_STOREID,
                                $storeId);
                            $this->session->unsetDraftIds($quoteItem->getProduct()
                                ->getId(), $storeId);
                        }
                    }
                }
