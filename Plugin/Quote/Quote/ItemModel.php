    ...
    public function aroundRepresentProduct(
        ...
        if ($result) {
            ...
            if (isset($itemOptions['info_buyRequest'])
                && $itemOptions['info_buyRequest'] instanceof
                \Magento\Quote\Model\Quote\Item\Option
            ) {
                $infoBuyRequest = $itemOptions['info_buyRequest'];
                $value
                                = $this->unserialize($infoBuyRequest->getValue());
                $itemDraftId
                                = isset($value[InstallSchema::COLUMN_NAME_DRAFTID])
            ...
            if (isset($productOptions['info_buyRequest'])
                && $productOptions['info_buyRequest'] instanceof
