...
class Cart extends AbstractHelper
{
    ...
    public function updateDraftHashRelations(
        ...
        if (is_array($draftHashRelations)) {
            ...
            if (is_array($draftHashRelations[$productId])) {
                $draftHashRelations[$productId][$printformerProductId]
                    = $draftId;
            }
        }
        ...
        if ($this->_request->getActionName() == 'configure'
            ...
            if ($id) {
                switch ($this->_request->getModuleName()) {
                    case 'checkout':
                        $quoteItem = $this->cartModel->getQuote()
                            ->getItemById($id);
                        if ($quoteItem
                            ...
                            if ($this->configHelper->useChildProduct($quoteItem->getProductType())) {
                                ...
                                if (!empty($children)) {
                                    ...
                                    if (!empty($firstChild)) {
                                        $draftId
                                            = $this->loadDraftFromQuoteItem($firstChild,
                                ...
                                $draftId
                                    = $this->loadDraftFromQuoteItem($quoteItem,
                                    $printformerProduct->getProductId(),
                                    $printformerProduct->getId());
                            }
                        }
                    ...
                    case 'wishlist':
                        $wishlistItem
                                    = $this->wishlistItemModel->loadWithOptions($id);
                        ...
                        if (!empty($draftField)) {
                            ...
                            foreach ($draftHashArray as $draftHash) {
                                $draftItem
                                    = $this->productHelper->getDraftById($draftHash);
                                if ($draftItem) {
                                    $pfProductId
                                        = $draftItem->getData('printformer_product_id');
                                    $productId
                                        = $draftItem->getData('product_id');
                                    if (!empty($productId)
                                        && !empty($pfProductId)
                                    ) {
                                        if ($pfProductId
                                            ...
                                            $draftField
                                                = $wishlistItem->getOptionByCode($this->productHelper::COLUMN_NAME_DRAFTID)
                                                ->getValue();
                                            if (!empty($draftField)) {
                                                $draftHashArray = explode(',',
                                                    $draftField);
                                                foreach (
                                                    $draftHashArray as
                                                    $draftHash
                                                    ...
                                                    $draft
                                                        = $this->productHelper->getDraftById($draftHash);
                                                    if ($draft->getPrintformerProductId()
                                                        == $pfProductId
                                                        && $draft->getProductId()
                                                        == $productId
                                                    ) {
            ...
            $draftId     = $this->productHelper->getDraftId($pfProductId,
                $productId);
    ...
    public function loadDraftFromQuoteItem(
        ...
        $resultDraft = false;
        if (($productId && $productId == $quoteItem->getProduct()->getId())
            || (empty($productId))
        ) {
            $quoteItemDraftsField
                = $quoteItem->getData(SessionHelper::SESSION_KEY_PRINTFORMER_DRAFTID);
            if (!empty($quoteItemDraftsField)) {
                $quoteItemDraftsCollectionItems
                    = $this->productHelper->loadDraftItemsByIds($quoteItemDraftsField);
                foreach (
                    $quoteItemDraftsCollectionItems as $quoteItemCollectionItem
                ) {
                    if ($quoteItemCollectionItem->getProductId() == $productId
                        && $quoteItemCollectionItem->getPrintformerProductId()
                        == $printformerProductId
                    ) {
        ...
        if (is_array($quoteItems)) {
            foreach ($quoteItems as $quoteItem) {
                if ($quoteItem->getProductType()
                    ...
                    if ($quoteItem) {
                        ...
                        if (!empty($children = $quoteItem->getChildren())) {
                            foreach ($children as $child) {
                                if ($child->getData($this->productHelper::COLUMN_NAME_DRAFTID)
                                    == $draftId
                                ) {
                                    $result = true;
                                }
                    ...
                    if ($quoteItem->getData($this->productHelper::COLUMN_NAME_DRAFTID)
                        == $draftId
                    ) {
                        $result = true;
                    }
