...
class Media extends AbstractHelper
{
    ...
    public function deleteImage(
        ...
        $mediaDir
                        = $this->filesystem->getDirectoryWrite(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA);
    ...
    {
        return str_replace('{type}', ($isThumbnail ? 'thumbs' : 'preview'),
            self::IMAGE_PATH);
    }
    ...
    public function createPreview(
        ...
        imagecopyresampled($out, $image, 0, 0, 0, 0, $width, $height, $width,
            $height);
    ...
    public function getThumbnail(
        ...
        if ($uniqueGetParam) {
            $thumbnailUrl
                = $this->_urlHelper->appendUniqueGetParam($thumbnailUrl);
        }
    ...
    public function getImageFilePath(
        ...
        $imagePath              = sprintf($imagePathDefaultString, $draftId,
            $page);
        ...
        if ($this->_config->isUseImagePreview()) {
            if (!empty($draftIds)) {
                ...
                foreach ($draftIds as $draftIdKey => $draftId) {
                    $printformerDraft
                           = $this->_apiHelper->getDraftUsagePageInfo($draftId,
                        $this->_apiHelper::DRAFT_USAGE_PAGE_INFO_PREVIEW);
                    $pages = $printformerDraft['pages'] ?? 1;

                    for ($index = 0; $index < $pages; $index++) {
                        try {
                            $imagePreviewFilePath
                                            = $this->getImageFilePath($draftId,
                                ($index + 1));
                            $additionalHash = '';
                            if (file_exists($imagePreviewFilePath)) {
                                $additionalHash = '?hash='
                                    .filemtime($imagePreviewFilePath);
                            }
                            $imagePreviewUrl     = $this->getImagePreviewUrl(($index
                                + 1), $draftId);
                            $fullImagePreviewUrl = $imagePreviewUrl
                                .$additionalHash;
        ...
        if ($this->_config->isUseImagePreview() && !empty($draftId)) {
            try {
                ...
                if (!file_exists($filePath)
                    || !is_array(getimagesize($filePath))
                ) {
                    $jpgImg           = $this->_apiHelper->getThumbnail(
                        $draftId,
                        $this->_apiHelper->getUserIdentifier(),
                    ...
                    $printformerImage = $jpgImg['content'];
                    $imageFilePath    = $this->getImageFilePath($draftId,
                        $page);
                    $image
                                      = imagecreatefromstring($printformerImage);
                    $width            = imagesx($image);
                    $height           = imagesy($image);
                    $out              = imagecreatetruecolor($width, $height);
                    imagealphablending($out, false);
                    $transparentindex = imagecolorallocatealpha($out, 0, 0, 0,
                        127);
                    imagefill($out, 0, 0, $transparentindex);
                    imagesavealpha($out, true);
                    imagecopyresized($out, $image, 0, 0, 0, 0, $width, $height,
                        $width, $height);
    ...
    public function getImageUrl(
        ...
        $thumbnailUrl = $this->storeManager->getStore()
                ->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA)
            .sprintf($imagePath, $draftId, $page);
    ...
    public function loadThumbsImageUrlByDraftId(string $draftIds)
    {
        ...
        if ($this->_config->isUseImagePreview()) {
            if (!empty($draftIds)) {
                ...
                try {
                    if (!file_exists($this->getImageFilePath($draftIds, 1,
                        true))
                    ) {
