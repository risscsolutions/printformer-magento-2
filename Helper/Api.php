...
class Api extends AbstractHelper
{
    ...
    public function updateDraftHash($draftHash, $orderId, $storeId = null)
    {
        ...
        if ($this->_sessionHelper->hasDraftInCache($response['data']['draftHash'])) {
            $this->_sessionHelper->updateDraftInCache($response['data']['draftHash'],
                $response['data']);
        } else {
            $this->_sessionHelper->addDraftToCache($response['data']['draftHash'],
                $response['data']);
        }
    ...
    public function getHttpClient($storeId = false, $websiteId = false)
    {
        ...
        if (!isset($this->_httpClients[$storeId])) {
            $this->_httpClients[$storeId] = new Client([
                'base_url' => $this->apiUrl()->getPrintformerBaseUrl($storeId,
                    $websiteId),
                'headers'  => [
                    'Accept'        => 'application/json',
                    'Authorization' => 'Bearer '
                        .$this->_config->getClientApiKey($storeId, $websiteId),
                ],
            ]);
        }
    ...
    public function getDraftUsagePageInfo(
        ...
        $storedDraftPageInfo
            = $this->_sessionHelper->getDraftPageInfo($draftHash);
        ...
        if (!$storedDraftPageInfo) {
            ...
            $response = json_decode($response->getBody(), true);
            $this->_sessionHelper->setDraftPageInfo($draftHash,
                $response['data']);
    ...
    public function uploadPdf($draftId, $downloadableLinkFilePath)
    {
        $absoluteMediaPath
            ...
            ->getAbsolutePath();
        $absoluteDownloadableMediaPath = $absoluteMediaPath
            .'downloadable/files/links';
        $absoluteLinkFilePath          = $absoluteDownloadableMediaPath
            .$downloadableLinkFilePath;

        if (file_exists($absoluteLinkFilePath)
            && is_readable($absoluteLinkFilePath)
        ) {
            ...
            $downloadableTempLinkFilePath     = DirectoryList::TMP
                .DIRECTORY_SEPARATOR.self::API_UPLOAD_INTENT.DIRECTORY_SEPARATOR
                .$draftId.DIRECTORY_SEPARATOR
                .basename($downloadableLinkFilePath);
            $fullDownloadableTempLinkFilePath = $absoluteMediaPath
                .$downloadableTempLinkFilePath;
            if (!file_exists($fullDownloadableTempLinkFilePath)) {
                $directoryWriteInstance->copyFile($absoluteLinkFilePath,
                    $downloadableTempLinkFilePath);
            }
            ...
            $filePathUrl                = $baseUrl.DirectoryList::PUB
                .DIRECTORY_SEPARATOR.DirectoryList::MEDIA.DIRECTORY_SEPARATOR
                .$downloadableTempLinkFilePath;
            $callBackUrl                = $this->getUploadCallbackUrl($draftId);
            $callBackUrlWithQueryString = $callBackUrl.'?filepath='
                .$downloadableTempLinkFilePath;

            if (isset($filePathUrl) && isset($apiUrl)
                && isset($callBackUrlWithQueryString)
            ) {
                $this->_logger->notice('Used callbackUrl='
                    .$callBackUrlWithQueryString);
                ...
                try {
                    $createdEntry = $this->_logHelper->createPostEntry($apiUrl,
                        $requestData);
                    $response     = $this->getHttpClient()->post($apiUrl,
                        $requestData);
                    $this->_logHelper->updateEntry($createdEntry, [
                        'response_data' => $response->getBody()->getContents(),
                    ]);
                    ...
                    if ($response->getStatusCode() === 204) {
                        $this->_logger->debug('Upload status code: 204'
                            .'for upload with draft id: '.$draftId);

                        return true;
                    } else {
                        $this->_logger->debug('Upload status code: '
                    ...
                    $this->_logger->debug('Upload failed for draft with draft-id: '
                        .$draftId.' on file path order-id'.$filePathUrl
                        .' for callback url: '.$callBackUrlWithQueryString);
                    $this->_logger->debug('Upload error message: '
                        .$e->getMessage());
                }
            } else {
                return false;
    ...
    public function getUploadCallbackUrl($draftId)
    {
        ...
        return $this->urlBuilder->getUrl(self::CALLBACK_UPLOAD_ENDPOINT,
            $params);
    }
     ...
     * @throws Exception
     */
    public function getEditorWebtokenUrl(
        $draftHash,
        $userIdentifier,
        $params = []
    ) {
        ...
        $client
                        = $this->_config->getClientIdentifier($this->getStoreId());
        ...
        $entry
                                 = $this->_logHelper->createRedirectEntry($editorOpenUrl,
            $data);

        $redirectUrl = $this->apiUrl()->getAuth().'?'
            .http_build_query(['jwt' => $JWT]);
        $entry->setResponseData(json_encode([
    ...
    public function updateColorVariationOnProcess(
        ...
        $process = null;
        try {
            $process = $this->getDraftProcess($draftHash, $productId, $intent,
                $sessionUniqueId);
            if ($process->getId()) {
                $process->addData([
    ...
    public function getDraftProcess(
        ...
        if ($draftHash !== null) {
            $draftCollection->addFieldToFilter('draft_id',
                ['eq' => $draftHash]);
        } else {
            if ($intent !== null) {
                $draftCollection->addFieldToFilter('intent', ['eq' => $intent]);
            }
            $draftCollection->addFieldToFilter('session_unique_id',
                ['eq' => $sessionUniqueId]);
            $draftCollection->addFieldToFilter('product_id',
                ['eq' => $productId]);
        }
        if ($printformerProductId !== null) {
            $draftCollection->addFieldToFilter('printformer_product_id',
                ['eq' => $printformerProductId]);
        }
    ...
    public function uploadDraftProcess(
        ...
        $process = $this->getDraftProcess($draftHash, $productId,
            self::API_UPLOAD_INTENT, $sessionUniqueId);
        if (!$process->getId() && !$checkOnly) {
            if (!$draftHash) {
                ...
                if ($this->_config->getOrderDraftUpdate($storeId)
                ...
                $dataParams = array_merge($dataParams,
                    $additionalUploadDataParams);
                $draftHash  = $this->createDraftHash($masterId,
                    $printformerUserIdentifier, $storeId, $dataParams);
     ...
     * @return mixed
     */
    public function createDraftHash(
        $masterId,
        $userIdentifier,
        $storeId,
        $params = []
        ...
        $createdEntry    = $this->_logHelper->createPostEntry($url,
            $requestData);
        $response        = $this->getHttpClient($storeId)->post($url,
            $requestData);
        ...
        $this->_logHelper->updateEntry($createdEntry, [
            ...
            'draft_id'      => $response['data']['draftHash'],
        ]);
    ...
    {
        $params
            = $this->printformerProductAttributes->mergeFeedIdentifier($params);
    ...
    {
        try {
            $draftProcessingUrl = $this->apiUrl()
                ->getDraftProcessing($draftIds);

            $stateChangedNotifyUrl
                         = $this->_urlBuilder->getUrl('rest/V1/printformer')
            ...
            $createdEntry
                      = $this->_logHelper->createPostEntry($draftProcessingUrl,
                $requestData);
            $response = $this->getHttpClient($storeId)
                ->post($draftProcessingUrl, $requestData);
            $this->_logHelper->updateEntry($createdEntry,
                ['response_data' => $response->getBody()->getContents()]);
        } catch (Exception $e) {
            $this->_logger->debug('Process for draft ids failed. Error-message: '
                .$e->getMessage());
        }

        if (!empty($response)) {
            $responseArray  = json_decode($response->getBody(), true);
            $processingHash = !empty($responseArray['processingId'])
                ? $responseArray['processingId'] : null;
            if (!empty($processingHash)) {
                ...
                foreach ($draftIds as $draftHash) {
                    try {
                        ...
                        $this->_logger->debug('Error on draft processing for draft: '
                            .$draftHash.PHP_EOL.'Status-code: '.$e->getCode()
                            .PHP_EOL.$e->getMessage().'Line: '.$e->getLine()
                            .PHP_EOL.'File: '.$e->getFile());
                    }
                }

                $this->_logger->debug('Drafts processing failed: '.implode(",",
                        $draftIdsToProcessFailed));
                $this->_logger->debug('Drafts processing successfully processed: '
                    .implode(",", $draftIdsToProcessSuccess));
            }
        } else {
            $this->_logger->debug('Cant get response for draft-ids:'
                .implode(",", $draftIds));
        }
    }
     ...
     * @param $draftId
     */
    public function setProcessingStateOnOrderItemByDraftId(
        $draftId,
        $printformerCountState
    ) {
        ...
        if (isset($orderItem) && $orderItem->getItemId() !== null) {
            ...
            $item->setPrintformerCountDate($this->timezone->date()
                ->format('Y-m-d H:i:s'));
    ...
    public function getReplicateDraftId(string $oldDraftId): string
    {
        ...
        if ($this->_sessionHelper->hasDraftInCache($draftHash)) {
            $this->_sessionHelper->updateDraftInCache($draftHash,
                $draftInfo['data']);
        } else {
            $this->_sessionHelper->addDraftToCache($draftHash,
                $draftInfo['data']);
        }
    ...
    public function draftProcess(
        ...
        if (!$process->getId() && !$checkOnly) {
            ...
            if (!$draftHash) {
                try {
                    $draftHash = $this->createDraftHash($masterId,
                        $this->getUserIdentifier(), $storeId, $dataParams);
            ...
            if ($store->getCode() == 'admin') {
                if ($admin !== null) {
                    ...
                    ");
                        ...
                        if ($admin->getData('printformer_identification')
                            != $this->_adminSession->getPrintformerIdentification()
                        ) {
        ...
        if ($customer->getPrintformerIdentification() !== null) {
            ...
            if ($response->getStatusCode() === 200) {
                ...
                if ($profileData['firstName'] == ''
                    ...
                    $createdEntry = $this->_logHelper->createPutEntry($url,
                        $requestData);
    ...
    public function getThumbnail(
        ...
        try {
            ...
            $completeThumbnailUrl = $thumbnailUrl.'?'
                .http_build_query($postFields);

            $createdEntry
                      = $this->_logHelper->createGetEntry($completeThumbnailUrl);
    ...
    public function migrateDrafts(
        ...
        $createdEntry = $this->_logHelper->createPostEntry($url, $requestData);
        $body         = $this->getHttpClient()->post($url, $requestData)
            ->getBody();
        $this->_logHelper->updateEntry($createdEntry,
            ['response_data' => $body->getContents()]);
    ...
    public function userMerge($userIdentifierOne, $userIdentifierTwo)
    {
        ...
        $url = $this->apiUrl()->getPrintformerBaseUrl().'/api-ext/user/'
            .$userIdentifierOne.'/merge';
    ...
    public function getPrintformerDraft($draftHash, $forceUpdate = false)
    {
        if (!$this->_sessionHelper->hasDraftInCache($draftHash)
            ...
            $response = json_decode($response->getBody(), true);
            if ($forceUpdate
                && $this->_sessionHelper->hasDraftInCache($response['data']['draftHash'])
            ) {
                $this->_sessionHelper->updateDraftInCache($response['data']['draftHash'],
                    $response['data']);
            } else {
