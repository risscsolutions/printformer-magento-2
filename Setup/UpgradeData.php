...
class UpgradeData implements UpgradeDataInterface
{
    ...
    public function upgrade(
        ...
        if (version_compare($context->getVersion(), '100.3.8', '<')) {
            ...
            foreach ($result as $row) {
                foreach (explode(',', $row['intent'] ?? '') as $intent) {
                    if ($i == 1000) {
                        $connection->beginTransaction();
                        $connection->insertMultiple('printformer_product',
                            $insertData);
                        $connection->commit();
                        $insertData = [];
                        $i          = 0;
        ...
        if (version_compare($context->getVersion(), '100.6.11', '<')) {
            ...
            if (!$eavSetup->getAttribute(Customer::ENTITY,
                'printformer_identification')
            ) {
                $this->addPrintformerIdentificationAttribute($eavSetup);
            }

            $eavSetup->updateAttribute(Customer::ENTITY,
                'printformer_identification', 'is_visible', false);
        }
        ...
        if (version_compare($context->getVersion(), '100.8.20', '<')) {
            ...
            $eavSetup->removeAttribute(\Magento\Catalog\Model\Product::ENTITY,
                'feed_identifier');
            $categorySetup
                = $this->categorySetupFactory->create(['setup' => $setup]);
            $eavSetup->addAttribute(
                \Magento\Catalog\Model\Product::ENTITY,
                'feed_identifier',
                [
            ...
            $attributeSetId
                                = $categorySetup->getDefaultAttributeSetId(\Magento\Catalog\Model\Product::ENTITY);
