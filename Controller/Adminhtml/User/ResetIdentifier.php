        ...
        try {
            ...
            if ($websiteId !== null) {
                if ($this->_gateway->deletePrintformerIdentification($websiteId)) {
                    $response = [
                        'success' => true,
                        'message' => __('Printformer Identifiers have been removed for all Stores from this Website.'),
                    ];
                } else {
                    $response = [
                        'success' => false,
                        'message' => __('Can\'t reset printformer_identification for %1.',
                            $websiteId),
                    ];
                }
            }
            ...
            if ($storeId != "0") {
                if ($this->_gateway->deletePrintformerIdentificationByStoreId($storeId)) {
                    $response = [
                        'success' => true,
                        'message' => __('Printformer Identifiers have been removed for Store with ID %1',
                            $storeId),
                    ];
                } else {
                    $response = [
            ...
            if (($storeId === 0 || $storeId == 0) && $websiteId === null) {
                if ($this->_gateway->deletePrintformerIdentification(null)) {
                    $response = [
                        'success' => true,
                        'message' => __('All Printformer Identifiers have been removed'),
                    ];
                } else {
                    $response = [
                        'success' => false,
                        'message' => __('Can\'t reset printformer_identification.'),
                    ];
                }
            }
        } catch (\Exception $e) {
