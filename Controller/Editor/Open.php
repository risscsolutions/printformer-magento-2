...
class Open extends Action
{
    ...
    public function execute()
    {
         ...
         */
        if (!$overrideFrameConfig
            && $this->_apiHelper->config()->isFrameEnabled()
        ) {
    ...
    protected function _getParam(string $key)
    {
        ...
        if ($sessionParams = $this->_sessionHelper->getCatalogSession()
            ->getPrintformerRedirectParameters()
        ) {
    ...
    {
        if ($this->_sessionHelper->getCatalogSession()
            ->getPrintformerRedirectParameters()
        ) {
            return null;
        }
        ...
        $this->_sessionHelper->getCatalogSession()
            ->setPrintformerRedirectParameters($dataParams);
    ...
    protected function _clearSession()
    {
        ...
        $this->_sessionHelper->getCatalogSession()
            ->setPrintformerRedirectParameters(null);
    }
    ...
    protected function initShopFrame()
    {
        ...
        $iframeBlock = $resultPage->getLayout()
            ->getBlock('printformer_editor_shopframe');
    ...
    protected function _createDraftProcess(
        ...
        $draftId
                        = $this->_draftGateway->createDraft($product->getPrintformerProduct(),
            $intent);
    ...
    protected function _getDraftProcess(
        ...
        if ($draftCollection->count() == 1) {
            if ($draftCollection->getFirstItem()->getUserIdentifier()
                == $this->_draftGateway->getUserIdentifier()
    ...
    protected function _buildRedirectUrl(
         ...
         */
        $editorUrlParamsArray['callback']
            = $this->_getCallbackUrl($requestReferrer, $draftProcess, $storeId,
            $params);
    ...
    protected function _getCallbackUrl(
        ...
        if ($requestReferrer != null) {
            ...
            $referrer = $this->_url->getUrl('printformer/editor/save',
                $referrerParams);
        }
